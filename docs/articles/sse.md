...
`Open` 不表示已经收到 body，也不表示已满足下节的退避重置条件。对没有重定向的常规请求，超时边界分为两个连续阶段：
...
自动跟随重定向时，`IdleTimeout` 在首个请求即将发送后保持覆盖整个逻辑请求，不会在中间 3xx 响应头结束时暂停。原因是 libcurl 为复用连接可能继续读取并丢弃中间响应体；此过程不会交付给 SSE 的 body 回调，过早暂停会留下无超时窗口。libcurl 仍会为后续每一跳重新应用 `ConnectTimeoutMs`，因此后续 DNS、连接及协议协商阶段由两者共同约束，先到者生效；下一跳请求即将发送时会再次续期 `IdleTimeout`。
...
> `204` 与 requestFactory 返回非法 SSE 请求始终终止，不经过 `ShouldReconnect`。
>
...
- **fault 原始异常**：requestFactory 返回非法 SSE 请求、某个用户回调（`OnEvent`/`OnError`/`OnStateChanged`）抛出，或后台循环遇到未预期异常。
