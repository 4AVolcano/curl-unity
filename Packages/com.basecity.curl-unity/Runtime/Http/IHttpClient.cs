...
{
    /// <summary>
    /// HTTP 客户端接口。负责接收 <see cref="HttpRequest"/> 并异步返回 <see cref="IHttpResponse"/>,
    ...
    public interface IHttpClient : IDisposable
    {
        ...
        /// 释放底层 easy handle。
        /// </summary>
        /// <param name="request">请求配置。不可为 null,<see cref="HttpRequest.Url"/> 必填。</param>
        /// <param name="ct">取消 token。请求已在飞行时取消会把 Task 置为 Canceled 并向
        /// libcurl 发取消,不等待网络。</param>
        Task<IHttpResponse> SendAsync(HttpRequest request, CancellationToken ct = default);
